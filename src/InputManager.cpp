...
InputManager::InputManager(Gyvr::Util::SharedContext& p_sharedContext)
...
{
	const auto result = m_sharedContext.window->GetDevice()->GetKey(static_cast<int>(p_key));
...
{
	const auto result = m_sharedContext.window->GetDevice()->GetMouseButton(static_cast<int>(p_mouseButton));
...
Gyvr::Math::Vec2 InputManager::GetCursorPosition() const
{
	const std::pair<double, double> mousePos = m_sharedContext.window->GetDevice()->GetCursorPosition();
	return Math::Vec2(static_cast<float>(mousePos.first), static_cast<float>(mousePos.second));
}
